...
void followTrajectory() {
    ...
    if (freshWirelessData) {
        ...
        double strafe = abs(controllerMessage.joystick1.x) < 0.1 ? 0 : mapDouble(controllerMessage.joystick1.x, -1, 1, -MAX_FORWARD, MAX_FORWARD);
        double turn = abs(controllerMessage.joystick2.x) <0.1 ?0: mapDouble(controllerMessage.joystick2.x, -1,1, -MAX_TURN, MAX_TURN);//want our strafe to be as fast as our forward basically? 
         //Front left, back left, front right, Bk right
       updateSetpoints(forward+strafe+turn, forward -strafe+turn, forward - strafe-turn,forward+strafe-turn);
    } 
    #endif 
