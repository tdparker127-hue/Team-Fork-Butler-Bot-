...
# Simple target-tag servoing.
TARGET_TAG_ID = 1
TARGET_STOP_DISTANCE_M = 0.65
TARGET_FORWARD_TOL_M = 0.05
TARGET_CENTER_TOL_NORM = 0.06
...
MAX_FORWARD_CMD = 0.18
MAX_YAW_CMD = 0.35
TURN_SLOWDOWN_START_NORM = 0.20
# ---------------------------------------------------------------------------
...
def _draw_overlay(frame, raw_result, ekf_pose, fps: float,
                  enc_data: dict, imu_data: dict,
                  nav_status: dict | None = None) -> None:
    ...
    if nav_status is not None:
        put(frame, f"target id: {nav_status['tag_id']}", 5, col_r, (180, 230, 255))
        if nav_status["visible"]:
            put(frame, f"tag x={nav_status['tag_x']:+.2f}  y={nav_status['tag_y']:+.2f} m",
                6, col_r)
            put(frame, f"img err={nav_status['img_err_px']:+.0f}px  yaw={nav_status['yaw_cmd']:+.2f}",
                7, col_r)
            put(frame, f"fwd err={nav_status['forward_err']:+.2f} m  ly={nav_status['forward_cmd']:+.2f}",
                8, col_r)
            state_color = (120, 255, 120) if nav_status["at_goal"] else (255, 220, 120)
            state_label = "AT GOAL" if nav_status["at_goal"] else "TRACKING"
            put(frame, f"state: {state_label}", 9, col_r, state_color)
        else:
            put(frame, "state: target not visible", 6, col_r, (80, 80, 255))
...
def _robot_frame_tag_pose(det) -> np.ndarray | None:
    """Return the detected tag position in the robot frame [right, forward, up]."""
    if det.pose_t is None:
        return None
    p_cam = np.array(det.pose_t, dtype=float).ravel()
    return T_CAM_ROBOT[:3, :3] @ p_cam + T_CAM_ROBOT[:3, 3]


def _find_target_detection(detections, target_tag_id: int):
    for det in detections:
...
def _compute_target_nav(target_det, frame_w: int, dt: float,
                        prev_forward_err: float | None,
                        prev_x_err_norm: float | None) -> dict | None:
    tag_pos = _robot_frame_tag_pose(target_det)
    if tag_pos is None:
        return None

    tag_center_x = float(np.mean(target_det.corners[:, 0]))
    frame_center_x = 0.5 * frame_w
    img_err_px = tag_center_x - frame_center_x
    x_err_norm = img_err_px / max(frame_center_x, 1.0)
    forward_err = float(tag_pos[1] - TARGET_STOP_DISTANCE_M)

    if dt <= 1e-6:
        x_err_rate = 0.0
        forward_rate = 0.0
    else:
        x_err_rate = 0.0 if prev_x_err_norm is None else (x_err_norm - prev_x_err_norm) / dt
        forward_rate = 0.0 if prev_forward_err is None else (forward_err - prev_forward_err) / dt

    yaw_cmd = _clip(YAW_KP * x_err_norm + YAW_KD * x_err_rate,
                    -MAX_YAW_CMD, MAX_YAW_CMD)

    if forward_err <= TARGET_FORWARD_TOL_M:
        forward_cmd = 0.0
        at_goal = True
    else:
        forward_cmd = _clip(
            FORWARD_KP * forward_err + FORWARD_KD * forward_rate,
            0.0,
            MAX_FORWARD_CMD,
        )
        at_goal = False

    if abs(x_err_norm) > TURN_SLOWDOWN_START_NORM:
        turn_scale = 1.0 - min(1.0, (abs(x_err_norm) - TURN_SLOWDOWN_START_NORM) /
                               max(1e-6, 1.0 - TURN_SLOWDOWN_START_NORM))
        forward_cmd *= max(0.2, turn_scale)

    centered = abs(x_err_norm) <= TARGET_CENTER_TOL_NORM
    return {
        "tag_id": int(target_det.tag_id),
        "tag_x": float(tag_pos[0]),
        "tag_y": float(tag_pos[1]),
        "tag_z": float(tag_pos[2]),
        "img_err_px": float(img_err_px),
        "img_err_norm": float(x_err_norm),
...
def main():
    ...
    raw_result = None
    prev_forward_err = None
    prev_x_err_norm = None

    print(f"Test mode: {TEST_MODE.value}")
    print(f"Tracking AprilTag {TARGET_TAG_ID} with slow forward + yaw PD control.")
    ...
    while True:
        ...
        target_det = _find_target_detection(detections, TARGET_TAG_ID)
        if target_det is not None:
            nav_result = _compute_target_nav(
                target_det,
                frame.shape[1],
                dt,
                prev_forward_err,
            ...
            if nav_result is not None:
                nav_status = nav_result
                prev_forward_err = nav_result["forward_err"]
                prev_x_err_norm = nav_result["img_err_norm"]

                if rc._drive_ser is not None:
                    rc.send_drive(0.0, nav_result["forward_cmd"], nav_result["yaw_cmd"])

                target_center = target_det.corners.astype(int)
                cv2.polylines(frame, [target_center.reshape(-1, 1, 2)], True, (0, 255, 255), 3)
                tag_cx = int(np.mean(target_det.corners[:, 0]))
                tag_cy = int(np.mean(target_det.corners[:, 1]))
                cv2.circle(frame, (tag_cx, tag_cy), 6, (0, 255, 255), -1)
                cv2.line(frame,
                         (frame.shape[1] // 2, 0),
        ...
        if ENABLE_CV_WINDOW:
            ...
            _draw_overlay(frame, raw_result, (ex, ey, etheta), fps, enc_data, imu_data, nav_status)
